...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_no_telp = $conn->query("SELECT * FROM users WHERE no_telp='$no_telp'");

    if ($check_email->num_rows > 0) {
        $register_message = "Email sudah terdaftar!";
        $message_type = "error";
    } elseif ($check_no_telp->num_rows > 0) {
        $register_message = "Nomor Telepon sudah terdaftar!";
        $message_type = "error";
    } else {
        $insert_query = "INSERT INTO users (email, password, no_telp, nama) VALUES ('$email', '$password', '$no_telp', '$nama')";
        if ($conn->query($insert_query) === TRUE) {
            $register_message = "Registrasi berhasil. Silakan <a href='login.php' class='font-semibold text-indigo-900 hover:underline'>Login</a>";
...
<head>
    ...
    <style>
        ...
        .register-container { /* Ganti dari .login-container untuk kejelasan */
        ...
        .btn-register:hover { /* Ganti dari .btn-login untuk kejelasan */
            background-color: #121258;
            transform: translateY(-1px);
        }
        
        .btn-register:active { /* Ganti dari .btn-login untuk kejelasan */
        ...
        @media (max-width: 640px) {
            .register-box { /* Ganti dari .login-box untuk kejelasan */
                width: 90%;
                padding: 1.5rem;
            }
        }
...
<body class="min-h-screen flex items-center justify-center bg-cover bg-center" style="background-image: url('https://images.unsplash.com/photo-1494976388531-d1058494cdd8?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80');">
    <div class="register-container absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
        <div class="register-box bg-white rounded-xl shadow-xl p-8 w-full max-w-md transition-all duration-300">
            ...
            <h1 class="text-2xl font-bold text-center text-gray-800 mb-8">Daftar Akun Baru</h1>
